...
describe('TeamForm', () => {
    ...
    describe('新規作成モード', () => {
        it('空のフォームが表示される', () => {
            render(
                <TeamForm onSubmit={mockOnSubmit} onCancel={mockOnCancel} />
            );

            expect(screen.getByLabelText('チーム名')).toHaveValue('');
            expect(screen.getByText(`選手 (6/${API_CONSTRAINTS.MAX_PLAYERS_PER_TEAM})`)).toBeInTheDocument();
        });
    ...
    describe('編集モード', () => {
        ...
        it('既存データがフォームに表示される', () => {
            ...
            expect(screen.getByText(`選手 (2/${API_CONSTRAINTS.MAX_PLAYERS_PER_TEAM})`)).toBeInTheDocument();
        });
    ...
    describe('選手の追加・削除', () => {
        it('選手を追加できる', async () => {
            ...
            expect(screen.getByText(`選手 (7/${API_CONSTRAINTS.MAX_PLAYERS_PER_TEAM})`)).toBeInTheDocument();
        });
        ...
        it('選手を削除できる', async () => {
            ...
            expect(screen.getByText(`選手 (5/${API_CONSTRAINTS.MAX_PLAYERS_PER_TEAM})`)).toBeInTheDocument();
        });
        ...
        it('14人に達すると追加ボタンが非表示になる', async () => {
            ...
            expect(screen.getByText(`選手 (${API_CONSTRAINTS.MAX_PLAYERS_PER_TEAM}/${API_CONSTRAINTS.MAX_PLAYERS_PER_TEAM})`)).toBeInTheDocument();
