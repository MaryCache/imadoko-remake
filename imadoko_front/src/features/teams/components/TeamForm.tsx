...
export const TeamForm: React.FC<TeamFormProps> = ({ initialData, onSubmit, onCancel }) => {
    ...
    const [players, setPlayers] = useState<Omit<Player, 'id'>[]>(
        ...
        Array(6).fill(null).map(() => ({ ...EMPTY_PLAYER }))
    );
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const validPlayers = sanitizedPlayers.filter(p => 
                p.firstName.trim() && p.lastName.trim() && p.position
            );
            ...
            const errorMsg = apiError?.response?.data?.message || 'フォームの送信に失敗しました';
    ...
    return (
        <form onSubmit={handleSubmit} className="space-y-8 bg-white p-6 rounded-xl shadow-sm border border-slate-200" aria-label={initialData ? 'チーム編集フォーム' : 'チーム作成フォーム'}>
            {submitError && (
            ...
            <div className="space-y-4">
                <div className="flex items-center justify-between mb-4">
                    <h3 className="text-lg font-semibold text-slate-900">選手 ({players.length}/{API_CONSTRAINTS.MAX_PLAYERS_PER_TEAM})</h3>
                    {players.length < API_CONSTRAINTS.MAX_PLAYERS_PER_TEAM && (
                        <Button type="button" variant="secondary" size="sm" onClick={addPlayer} className="gap-1">
                            <Plus size={16} />
                            選手を追加
                        </Button>
                    )}
                    ...
                    <AnimatePresence mode="popLayout">
                        {players.map((player, index) => (
                            ...
                            <div className="grid grid-cols-12 gap-3 flex-grow">
                                ...
                                <div className="col-span-4">
                                    <Listbox value={player.position} onChange={(value) => handlePlayerChange(index, 'position', value as Position)}>
                                        <div className="relative">
                                            ...
                                            <Transition
                                                ...
                                                leaveTo="opacity-0"
                                            >
                                                <Listbox.Options className={`absolute z-20 max-h-60 w-full overflow-auto rounded-xl bg-white p-1 text-base shadow-lg ring-1 ring-black/5 focus:outline-none sm:text-sm ${
                                                    index >= players.length - 2 ? 'bottom-full mb-1' : 'mt-1'
                                                }`}>
                                                    {POSITIONS.map((pos) => (
                                                        <Listbox.Option
                                                            key={pos.value}
                                                            value={pos.value}
                                                            className={({ active }) =>
                                                                `relative cursor-pointer select-none py-2 pl-3 pr-9 rounded-lg transition-colors ${
                                                                    active ? 'bg-mikasa-blue-light' : 'text-slate-900'
                                                                }`
                                                            }
                                                        >
                                                            {({ selected }) => (
