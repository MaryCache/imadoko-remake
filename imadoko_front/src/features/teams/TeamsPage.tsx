...
export const TeamsPage: React.FC = () => {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* エラー状態 */}
      {error && !isLoading && <ErrorMessage message={error} onRetry={actions.fetchTeams} />}

      {/* メインコンテンツ */}
      {!isLoading && !error && (
        <>
          {isCreating ? (
            <TeamForm onSubmit={handleCreate} onCancel={() => setIsCreating(false)} />
          ) : editingTeam ? (
            <TeamForm
            ...
            <TeamList teams={teams} onEdit={setEditingTeam} onDelete={handleDelete} />
          )}
        </>
      )}
    </div>
