...
export const CourtSlot = memo<CourtSlotProps>(
  ({ slot, player, side, hasServe, isFocused, onKeyDown, slotRef }) => {
      ...
      >
        ...
        <div className="absolute top-1.5 left-1.5" style={{ zIndex: Z_INDEX.SLOT_BADGE }}>
          <div className="w-6 h-6 sm:w-7 sm:h-7 rounded-full bg-white/30 backdrop-blur-sm flex items-center justify-center border border-white/50 shadow-sm">
            <span className="text-xs sm:text-sm font-bold text-white drop-shadow-sm">{slot}</span>
          </div>
        </div>
        ...
        {player && (
          ...
          >
            ...
            <PositionBadge
              ...
              className="mb-0.5 flex-shrink-0 scale-90 sm:scale-100 origin-bottom"
            />
        ...
        {!player && (
          <div className="text-white/30 text-[10px] sm:text-xs uppercase tracking-wider font-medium">
            EMPTY
          </div>
        )}
      </div>
