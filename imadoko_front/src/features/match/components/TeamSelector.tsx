...
export const TeamSelector: React.FC<TeamSelectorProps> = ({ value, teams, onChange, placeholder }) => {
    return (
        <Listbox value={value} onChange={onChange}>
            <div className="relative">
                {/* 修正: py-2.5 -> py-2 (モバイル) に変更、フォントサイズ調整 */}
                <Listbox.Button className="w-full px-3 py-2 sm:px-4 sm:py-2.5 rounded-xl border-2 border-sky-200/60 bg-white 
                    ...
                    <Listbox.Options className="absolute z-20 mt-1 max-h-60 w-full overflow-auto rounded-xl bg-white p-1 text-base shadow-lg ring-1 ring-black/5 focus:outline-none sm:text-sm">
                        ...
                        {teams.map((t) => (
                            <Listbox.Option
                                ...
                                value={t}
                                className={({ active }) =>
                                    `relative cursor-pointer select-none py-2 pl-10 pr-4 rounded-lg transition-colors ${active ? 'bg-sky-100 text-sky-900' : 'text-slate-700'
                                    }`
                                }
                            >
                                {({ selected }) => (
