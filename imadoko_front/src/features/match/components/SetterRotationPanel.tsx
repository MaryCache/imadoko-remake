 ...
 */
export const SetterRotationPanel: React.FC<SetterRotationPanelProps> = ({ team, side, onApplyRotation, currentAssignment }) => {
    ...
    const currentSetterPosition = selectedSetter
        ? (Object.entries(currentAssignment).find(
          ...
          )?.[0] ? Number(Object.entries(currentAssignment).find(
            ([_, player]) => player?.id === selectedSetter.id
          )?.[0]) : undefined)
        : undefined;
            ...
            <AnimatePresence>
                {isExpanded && (
                    ...
                    <div className="space-y-2">
                        ...
                        <Listbox value={selectedSetter} onChange={setSelectedSetter} disabled={setters.length === 0}>
                            <div className="relative">
                                <Listbox.Button className="w-full px-3 py-2.5 rounded-xl border-2 border-sky-200/60 bg-white text-slate-900 
                                    ...
                                    <span className="block truncate">
                                        {selectedSetter ? `${selectedSetter.lastName} ${selectedSetter.firstName}` : '-- セッターを選択 --'}
                                    </span>
                                ...
                                >
                                    <Listbox.Options className="absolute z-20 mt-1 max-h-60 w-full overflow-auto rounded-xl bg-white p-1 text-base shadow-lg ring-1 ring-black/5 focus:outline-none sm:text-sm">
                                        {setters.length === 0 ? (
                                            <div className="p-2 text-slate-500 text-xs text-center">候補がいません</div>
                                        ) : (
                                            setters.map((setter) => (
                                                <Listbox.Option
                                                    key={setter.id}
                    ...
                    <p className="text-xs text-slate-600 bg-white/60 p-2.5 rounded-lg border border-slate-200 leading-relaxed">
                        <span className="font-semibold text-indigo-700">💡 使い方:</span><br />
                        1. セッターを選択<br />
                        2. S1～S6のボタンをクリック → 即座にローテーション！<br />
